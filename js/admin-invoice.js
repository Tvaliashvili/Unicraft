window.downloadAdminInvoicePDF = function(rows, invoiceNo, dateStr, buyerName, buyerId) {
  ...
  const rowsHtml = rows.filter(function(r) {
    ...
    return '<tr>'
      + '<td>' + (r.name || '–') + '</td>'
      + '<td style="font-family:monospace;font-size:12px;color:#9ca3af">' + (r.code || '–') + '</td>'
      + '<td style="font-size:12px;color:#9ca3af">' + (r.store || '–') + '</td>'
  ...
  const html = '<!DOCTYPE html><html lang="ka">'
    ...
    + '.inv-label{font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#9ca3af;text-align:right}'
    ...
    + '.plabel{font-size:12px;color:#9ca3af;margin-bottom:2px}'
    ...
    + 'th{font-size:12px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#9ca3af;padding:0 4px 8px;text-align:left}'
    + 'th.r{text-align:right}th.c{text-align:center}'
    + 'td{font-size:14px;padding:8px 4px;border-bottom:1px solid #F5F2EE}'
    + '.vat-row td{font-size:12px;color:#9ca3af;padding:12px 4px 4px;border-bottom:none}'
    ...
    + '.total-lbl{font-size:12px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#9ca3af}'
    + '.paynote{margin-top:12px;padding:12px 0;font-size:12px;color:#9ca3af;text-align:center;line-height:1.625;border-top:1px solid #f9fafb}'
    + '.paynote strong{color:#141210;font-weight:500}'
    + '.disclaimer{margin-top:12px;padding:12px 16px;background:#F5F2EE;border:1px solid #e5e7eb;border-radius:8px;font-size:12px;color:#6b7280;line-height:1.625}'
    ...
    + '    <tfoot>'
    + '      <tr class="vat-row">'
    + '        <td colspan="5" style="text-align:right;padding-right:8px">მათ შორის დღგ (18%)</td>'
    + '        <td style="text-align:right">' + vat.toFixed(2) + ' ₾</td>'
    + '      </tr>'
    + '      <tr class="total-row">'
    + '        <td colspan="5" style="text-align:right;padding-right:8px"><span class="total-lbl">სულ გადასახდელი</span></td>'
